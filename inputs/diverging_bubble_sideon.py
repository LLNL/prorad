...
source_loc = [1.35,0.0,0.0]
prop_dir = [-1.0,0.0,0.0]

film_loc = [-27.0,0.0,0.0]
film_axis1 = [0.0,0.0,-3.0]
film_axis2 = [0.0,3.0,0.0]
...
l_prop = 0.4090 # distance after start (along prop_dir) through which to compute proton deflections
l_s2start = 1.1480 # distance from the source to start calculating proton deflections
...
ngridx, ngridy, ngridz = 400, 300, 150 # REQUIRED FOR ANALYTIC GRID

lx = 0.4050 # width of simulation box in cm (REQUIRED FOR ANALYTIC GRID)
ly = 0.4000 # height of simulation box in cm (REQUIRED FOR ANALYTIC GRID)
lz = 0.1400 # REQUIRED FOR ANALYTIC GRID

gridcorner = (-0.2050,-0.2000,0.0) # REQUIRED FOR ANALYTIC GRID
...
def fields(coord):
    ...
    if x < -0.2000:
        if (z % 0.0150 < 0.0050) or (y % 0.0150 < 0.0050):
            Z = 79.0
            N = 6.02401601e22
